...
export { PrereqBlock, CadFloorPlan, SiteAerial, CAD_ROOMS, SITE_DEP_OF, featById, STATUS_COLOR };
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}><span style={{ fontWeight: 700 }}>Date</span><span className="mono" style={{ fontWeight: 700 }}>{rangeLabel}{Math.round(lo) === DATA_DATE && lo === hi ? " · today" : ""}</span></div>
        <input type="range" min={minDay} max={maxDay} value={from} onChange={(e) => { setFrom(Number(e.target.value)); setTo(Number(e.target.value)); }} style={{ width: "100%" }} />
