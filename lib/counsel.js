...
export const DOCQA_SYSTEM = `You are COUNSEL answering a specific question about the construction document provided (text or image). Answer only from the document; if it does not address the question, say so plainly. Be concise and concrete, quote the relevant clause when useful. This is commercial analysis, not legal advice; for legal interpretation recommend an attorney.`;

// Key-free fallback: find the most relevant sentences in the document text.
export function localDocAnswer(text, q) {
  if (!text) return "Set OPENAI_API_KEY to ask questions about an uploaded or scanned document. For this key-free demo, load the sample contract and ask about it.";
  const sentences = text.replace(/\s+/g, " ").split(/(?<=[.;])\s+/).filter((s) => s.trim().length > 15);
  const qw = (q.toLowerCase().match(/[a-z]{4,}/g) || []);
  const scored = sentences
    .map((s) => { const sl = s.toLowerCase(); return { s, score: qw.reduce((a, w) => a + (sl.includes(w) ? 1 : 0), 0) }; })
    .filter((x) => x.score > 0)
    .sort((a, b) => b.score - a.score)
    .slice(0, 2);
  if (!scored.length) return "I couldn't find that in the document text. Try rephrasing, or set OPENAI_API_KEY for full question answering.";
  return "From the document: " + scored.map((x) => x.s.trim()).join(" ");
}
