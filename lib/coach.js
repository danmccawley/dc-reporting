...
export const SKILLS = [
  ...
  { key: "compliance", label: "Reading the compliance register (PROVOST)" },
];
...
export const LESSONS = [
  ...
  { id: "compliance", skill: "compliance", title: "Reading the compliance register", minutes: 4, levels: ["CM", "PM", "ADMIN"],
    why: "PROVOST tracks what the program is legally subject to. Knowing how to read it keeps a permit or an OSHA gap from stopping work.",
    steps: [
      "Each requirement is grouped by jurisdiction — OSHA, federal, state, local, environmental — and carries its citation.",
      "Status is Compliant, Monitoring, Action needed, or Overdue; open the item to see what work it affects.",
      "A few requirements gate activities — e.g., an air permit before generators, a building permit before steel sequence 2.",
      "Every requirement links to its LIBRARIAN-vetted authority; PROVOST surfaces and tracks, it is not legal advice.",
    ],
    tryIt: { label: "Open the compliance register", href: "/compliance" } },
];
...
const ROUTE_LESSON = [
  ...
  [/^\/library/, "source-credibility"], [/^\/forge/, "solutions-factory"], [/^\/compliance/, "compliance"],
];
...
const ROLE_GAPS = {
  CM: ["critical-path", "report-quality", "manpower", "lookahead", "rfi", "compliance", "earned-value", "solutions-factory"],
  PM: ["earned-value", "procurement", "compliance", "capacity", "rag-trend", "commissioning", "source-credibility", "solutions-factory", "critical-path"],
  CIM: ["capacity", "commissioning", "rag-trend", "source-credibility"],
  ADMIN: ["rag-trend", "earned-value", "critical-path", "report-quality", "compliance", "source-credibility", "solutions-factory"],
};
