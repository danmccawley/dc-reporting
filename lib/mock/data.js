    ...
    buildings.forEach((b) => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        entries.push({
          ...
          photos: Array.from({ length: i % 2 === 0 ? 3 : 2 }, (_, k) => ({ id: `${s.slug}-${bid}-${i}-${k}`, caption: `${name} · ${zone}`, date: DATES[i], tag: ["Progress", "As-built", "Constraint"][k] || "Photo" })),
        });
      }
      entries.push({
...
// ---------------------------------------------------------------------------
export const alerts = [
  { agent: "SENTINEL", severity: "r", text: "Switchgear lot for Building 17 at delivery risk. Typhoon tracking near supplier hub; QUARTERMASTER projects a 2 to 4 week slip that cascades into electrical fit-out and Cx.", href: "/schedule" },
  ...
  { agent: "KEYSTONE", severity: "r", text: "Building 16 commissioning is behind plan at 12%. L4 functional scripts gated on switchgear energization milestone.", href: "/commissioning" },
